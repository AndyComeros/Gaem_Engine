...
function init()
	...
	local scale = vec3:new(1.7,0.5,0.8)

	physics:AddRigidBodyColliderSphere(Player,0.5,vec3:new(1,0,0.5));
	physics:AddRigidBodyColliderSphere(Player,0.5,vec3:new(1,0,-0.5));
	physics:AddRigidBodyColliderSphere(Player,0.5,vec3:new(-1,0,0.5));
	physics:AddRigidBodyColliderSphere(Player,0.5,vec3:new(-1,0,-0.5));
	--physics:AddRigidBodyColliderCapsule(Player,0.8,1.8,vec3:new(0,0,0),vec3:new(0,0,90));
	--physics:AddRigidBodyColliderBox(Player,scale);
	--physics:AddRigidBodyColliderSphere(Player,0.5);
	...
	Player.rigidBody:SetAxisLinearFactor(1,0,1);
	--Player.rigidBody:SetAxisAngleFactor(0,1,1);
...
function update(deltaTime)
	...
	--print(height);
	Player:SetPosition(vec3:new(Player.position.x,height+1,Player.position.z));
